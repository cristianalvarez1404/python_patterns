...
@dataclass(frozen=True) #inmutable object
class HTMLPage:
  title: str
  ...
  def _render_metadata(self) -> str:
    return "\n".join(
      f'<meta name="{name}" content="{value}">' for name, value in self.metadata.items()
    )
...
class HTMLBuilder:
  ...
  def build(self) -> HTMLPage:
    ...
    return HTMLPage(title=self.title, body=body, metadata=self.metadata)