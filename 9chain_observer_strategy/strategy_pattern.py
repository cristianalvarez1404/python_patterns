...
class TravelRecommendation(BaseModel):
  ...
  message: str = Field(..., description = "Messsage from the travel agent")
...
def fun_agent() -> Agent[TravelDeps, TravelRecommendation]:
  return Agent(
    ...
    output_type = TravelRecommendation,
    system_prompt = (
      "You are fun, quirky travel agent who gets super excited about cool places. "
      "You responses are friendly and humorous, but still helpful."
    ),
  )
...
def budget_agent() -> Agent[TravelDeps, TravelRecommendation]:
  return Agent(
    ...
    output_type = TravelRecommendation,
    system_prompt = (
      "You are frugal travel expert who finds great destinations. "
      "You suggestions should highlight affordability and value."
    ),
  )

# Functions to Run Strategy
