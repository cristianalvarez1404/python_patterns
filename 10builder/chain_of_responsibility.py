...
hotel_agent = Agent(
  ...
  system_prompt = "Suggest a good hotel near the arrival airport or city center. Consider"
)
...
activity_agent = Agent(
  ...
  system_prompt = "Suggest local activities close to the hotel and suitable fro arrival"
)
...
async def handle_flight( user_input:str, deps:TravelDeps, ctx: TripContext
  ...
  ctx.from_city = flight_result.ouput.from_city
  ctx.arrival_time = flight_result.output.arrival_time
  print(
    f"🛫 Flight: from {ctx.from_city} -> {ctx.destination}, arriving at {ctx.arrival_time}"
  )
...
async def handle_hotel( user_input:str, deps:TravelDeps, ctx: TripContext
  ...
  ctx.hotel_name = hotel_result.ouput.name
  ctx.hotel_location = hotel_result.output.location
  print(
    f"🏨 Hotel: {ctx.hotel_name}, {hotel_result.output.starts}* at {hotel_result.output}"
  )
