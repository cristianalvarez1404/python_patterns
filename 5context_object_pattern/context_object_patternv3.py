...
def retrive_article(article_id: int, db: DBProtocol, logger: LoggerProtocol) -> Article | None:
...
def render_article(
   article: Article, logger:logging.Logger
) -> str:
...
def send_to_external_service(html: str, api_key: str):
...
def publish_article(article_id: int, context: AppContext):
  ...
  article = retrive_article(article_id, context.db, context.logger)
  if not article:
